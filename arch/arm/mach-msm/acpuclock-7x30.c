...
#define MAX_AXI_KHZ 203520

#define PLL2_L_VAL_ADDR  (MSM_CLK_CTL_BASE + 0x33c)

#define MIN_VOLTAGE_MV 750U
#define MAX_VOLTAGE_MV 1550U
...
static struct clkctl_acpu_speed acpu_freq_tbl[] = {
	...
	{ 230400, PLL_3,    5, 2,  122800, 900, VDD_RAW(900) },
        { 368640, PLL_3,    5, 1,  153600, 925, VDD_RAW(925) },
	{ 537600, PLL_2,    5, 1,  192000, 975, VDD_RAW(975) },
	...
	{ 1900800, PLL_2,   3, 0,  199680, 1500, VDD_RAW(1500) },
	{ 1920000, PLL_2,   3, 0,  199680, 1500, VDD_RAW(1500) },
	{ 1996800, PLL_2,   3, 0,  199680, 1525, VDD_RAW(1525) },
	{ 2016000, PLL_2,   3, 0,  201600, 1550, VDD_RAW(1550) },
#endif
	{ 0 }
};
...
static unsigned int acpuclk_get_current_vdd(void)
{
	...
	for (vdd_mv = MIN_VOLTAGE_MV; vdd_mv <= MAX_VOLTAGE_MV; vdd_mv += V_STEP)
		if (VDD_RAW(vdd_mv) == vdd_raw)
			break;

	if (vdd_mv > MAX_VOLTAGE_MV)
		return 0;
...
static int acpuclk_update_freq_tbl(unsigned int acpu_khz, unsigned int acpu_vdd)
{
	...
	if (acpu_vdd > MAX_VOLTAGE_MV || acpu_vdd < MIN_VOLTAGE_MV) {
...
{
if (khz == 0)
new_vdd = min(max((acpu_freq_tbl[i].vdd_mv + vdd), MIN_VOLTAGE_MV), MAX_VOLTAGE_MV);
else if (acpu_freq_tbl[i].acpu_clk_khz == khz)
new_vdd = min(max((unsigned int)vdd, MIN_VOLTAGE_MV), MAX_VOLTAGE_MV);
else continue;
